...
app
  .route("/task")
  .get(async (req, res) => {
    //查询task（uname或menmber右username）
    const { Collection: taskCollection, client } = await connectDB("task");
    const { uname, tname, tdetail, tdone, member } = req.query;
    if (!uname) {
      res.status(500).json({ message: "需要字段uname!" });
      return;
    }
    try {
      const result = await taskCollection.find({}).toArray();
      const resultIncludeuser = result.filter((item) => {
        return item.uname === uname || item.member.includes(uname);
      });
      res.json({ success: true, data: { result: resultIncludeuser } });
      client.close();
    } catch (e) {
      console.log(e);
  ...
  .post(async (req, res) => {
    ...
    const { uname, tname, tdetail, tdone, member } = req.body;
    if (!uname) {
  ...
  .put(async (req, res) => {
    //修改task
    const { Collection: taskCollection, client } = await connectDB("task");
    const { uname, _id, tname, tdetail, tdone, member } = req.body;
    if (!_id) {
      res.status(500).json({ message: "需要字段_id!" });
      return;
    }
    //const id = ObjectId(_id);
    const update = removeEmpty({ uname, tname, tdetail, tdone, member });
    try {
...
app
  ...
  .post(async (req, res) => {
    //新增user,username唯一,默认身份为u2(仅查看)
    const { Collection: userCollection, client } = await connectDB("user");
    userCollection.createIndex({ username: 1 }, { unique: true });
    const { username, password, permission } = req.body;
    console.log("postuser", username, password, permission);
    //可以选择权限
    try {
      const result = await userCollection.insertOne({
        username,
        password: encrypt(password),
        permission: permission || "u2",
      });
      res.json({ success: true, data: { result } });
      client.close();
    } catch (e) {
  ...
  .put(async (req, res) => {
    ...
    try {
      const setData = removeEmpty({ permission, password });
      if (setData.password) setData.password = encrypt(setData.password);
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (result.length) {
      ...
      res.json({ success: true, message: "登录成功", data: result[0] });
    } else {
